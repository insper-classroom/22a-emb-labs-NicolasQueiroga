...
void lv_termostato(void)
{
	...
	lv_obj_t *pwr = lv_btn_create(lv_scr_act());
	lv_obj_add_event_cb(pwr, event_handler, LV_EVENT_ALL, NULL);
	lv_obj_align(pwr, LV_ALIGN_BOTTOM_LEFT, 0, 0);
	lv_obj_add_style(pwr, &style, 0);
	lv_obj_set_width(pwr, 60);  lv_obj_set_height(pwr, 60);

	labelPwr = lv_label_create(pwr);
	lv_label_set_text(labelPwr, "[ " LV_SYMBOL_POWER);
	lv_obj_center(labelPwr);

	lv_obj_t *menu = lv_btn_create(lv_scr_act());
	lv_obj_add_event_cb(menu, event_handler, LV_EVENT_ALL, NULL);
	lv_obj_align_to(menu, pwr, LV_ALIGN_OUT_RIGHT_TOP, -5, 0);
	lv_obj_add_style(menu, &style, 0);
	lv_obj_set_width(menu, 60);  lv_obj_set_height(menu, 60);
